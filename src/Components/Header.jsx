...
const Header = () => {
  ...
  useEffect(() => {
    if (searchQuery !== "") {
      const temp = searchResults.filter((item) => {
        return item?.video?.title.includes(searchQuery);
      });
      setsuggestions(temp);
    } else setsuggestions([]);
  }, [searchQuery]);
  ...
  return (
    <div className={`${dark?"dark":""}`}>
      <div
        className="sticky top-0 z-10 flex flex-row items-center justify-between h-14 px-4 md:px bg-white dark:bg-black" 
      >
        {loading && <Loader />}
        ...
        <div
          className={`${
            suggestions.length > 0
              ? "flex flex-col pt-[335px] items-center"
              : ""
          }`}
        >
        ...
        <div className="flex items-center">
          <div className="hidden md:flex">
            ...
            >
              <CiDark className="text-black dark:text-white text-xl cursor-pointer" />
            </div>
